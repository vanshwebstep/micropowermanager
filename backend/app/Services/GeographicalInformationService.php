...
class GeographicalInformationService implements IBaseService, IAssociative {
    ...
    public function findByAssignee(Model $assignee): ?GeographicalInformation
    {
        return $this->geographicalInformation->newQuery()
            ->where('owner_type', $assignee->getMorphClass())
            ->where('owner_id', $assignee->getKey())
            ->first();
    }
